...
class ControllerExtensionPaymentCloudPayments extends Controller {
	...
	public function index($args) {
		...
		$data['show_button'] = $args['show_button'] ?? true;
	...
	private function processNotifyRequest($notify) {
		...
		switch ($notify) {
			...
			case self::NOTIFY_CONFIRM:
				...
				// Generate 1C Document
				$order_id = $order['order_id'];
				$this->load->model('fe/market/document');
				if ($this->model_fe_market_document ?? false) {
					$document = $this->model_fe_market_document->generateDocumentByOrderId($order_id);
					$this->model_fe_market_document->set($document);
				}

				$this->load->model('fe/util/telegram');
				if ($this->model_fe_util_telegram ?? false) {
					$this->model_fe_util_telegram->sendNotifications("Заказ #{$order_id} был оплачен.");
				}
